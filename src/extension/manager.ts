...
import { versionMatch } from '../utils-refactor';
import { GameWindow } from '../windows/game';

class MajsoulPlusExtensionManager {
  private loadedExtensions: { [extension: string]: string } = {};
    ...
    if (extension.dependencies) {
      for (const dep in extension.dependencies) {
        ...
        // dependency version mismatch
          ...
          !versionMatch(this.loadedExtensions[dep], extension.dependencies[dep])
        ) {
          console.error(
  ...
  useScript(folder: string, extension: MajsoulPlus.Extension) {
    ...
    if (!err) {
      this.loadedExtensions[extension.id] = extension.version;
    }
    return this;
  }
